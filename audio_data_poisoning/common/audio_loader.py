...
class AudioLoader:
    ...
    def _download_audios_in_batch(
        self,
        urls_batch: List[str],
        ranges_batch: List[Tuple[int, int]],
        captions_batch: List[str],
    ):
        for url, time_range, caption in zip(urls_batch, ranges_batch, captions_batch):
            with YoutubeDL(self._get_yt_dlp_params(time_range, caption)) as ydl:
                try:
                    ydl.download([url])
                except DownloadError as de:
                    print(f"Cannot download audio with url: {url}. {de}")

    def _get_yt_dlp_params(self, time_range: Tuple[int, int], caption: str) -> dict:
        return {
